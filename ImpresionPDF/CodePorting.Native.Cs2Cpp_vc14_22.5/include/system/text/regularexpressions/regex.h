...
public:
    ...
    /// @return Input strings with all matches replaced.
    ASPOSECPP_SHARED_API static String Replace(const String& input, const String& pattern, MatchEvaluator evaluator, RegexOptions options);
    ...
    /// Replaces substrings in string. Not implemented.
    static ASPOSECPP_SHARED_API String Replace(const String& input, const String& replacement, int count, int startat);
    ...
    /// Splits an input string a specified maximum number of times into an array of substrings, at the positions
    /// defined by a regular expression specified in the Regex constructor. The search for the regular expression
    /// pattern starts at a specified character position in the input string.
    /// @param input The string to be split.
    /// @param count The maximum number of times the split can occur.
    /// @param startat The character position in the input string where the search will begin.
    /// @return An array of strings.
    ASPOSECPP_SHARED_API ArrayPtr<String> Split(const String& input, int count, int startat);

    /// Converts regex to string.
    /// @return Regex pattern.
    String ToString() const override { return m_pattern; }
...
private:
    ...
    /// @return First match found.
    MatchPtr Match(const UStringPtr& source, int index);
